...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      const { data: cardInfo } = await admin
        .from("player_cards").select("name, gem_tiers(name)").eq("id", player_card_id).maybeSingle();
    ...
    } catch (e) { console.warn("[buy-gem-card] signing swallow", (e as Error).message); }
